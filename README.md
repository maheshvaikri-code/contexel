...
TypeScript/JavaScript agents get the same tool, same name, same behavior
([parity details](#typescript-parity)):
...
The [ts/](ts/) directory ships the same tool for TypeScript/JavaScript
agents, published to npm as
[`contexel`](https://www.npmjs.com/package/contexel). It is a **parity
port, not a re-imagining**: the Python implementation is canon, and both
languages run their test suites against the same golden vectors
...
and token counts must match byte-for-byte or the build is red.

What carries over exactly: all eight stages plus `merge`, `pipeline`/`stage`
composition, `shaped` (async tools are awaited, then shaped), `trace`/`audit`,
and code-point token counting (`"café"` costs the same in both languages;
`tokens.scoped` uses `AsyncLocalStorage` where Python uses `contextvars`).
The deliberate boundaries — JS collapses `1.0` to `"1"` in serialization,
Unicode word classes differ at the margins, pipeline fingerprints are
language-local in v1 — are recorded in
[ADR-001](docs/adr/001-typescript-parity.md).

...
| 6 | Not an injection defense — adversarial text outranked legitimate records by matching more query terms | **Controls shipped**: `allowlist` (fail-closed provenance gate, the strong control) + `quarantine` (deterministic tripwire for literal markers; paraphrase passes — honestly scoped, not a security boundary). Both traced/audited; the probe scenario is a regression test. Threat model stated in README + PyPI readme + site. Remaining roadmap: trust *tiers* (graded, not binary). |
